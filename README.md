...
**생중계 자막은 설정만으로 됩니다.** OpenAI 키를 넣고, 관리자 화면에서 생중계 주소를 등록하면
자막이 나옵니다 — 코드를 고칠 필요가 없습니다. 자세한 순서는 **[docs/SETUP.md](docs/SETUP.md)**.

| 무엇 | 어떻게 |
|---|---|
| **채널(위원회·영상 주소)** | 관리자 화면 `/admin/channels` 에서 등록합니다. 전국 48개 의회(광역 17 + 경기 시·군 31)의 생중계 페이지 주소가 들어 있어, 고르기만 하면 영상 주소를 찾아 줍니다 |
| **방송 중인지 판정** | 기본값은 영상 주소를 직접 확인하는 방식이라 기관 API 가 없어도 됩니다. 관리자가 직접 켜고 끄는 방식도 있습니다 |
| **기관 이름·브랜딩** | `ORG_NAME` 환경변수 + `frontend/public/ci/` 의 CI 교체 (경기도의회 CI 는 상표라 사용 불가 — `NOTICE` 참조) |
| **의원 명부** | `councilors` 표에 넣습니다. 비면 자막은 나오지만 **화자 이름이 붙지 않습니다** |

**설정을 비우면 자동으로 꺼지는 것** — 영상관리시스템(다시보기·속기록·안건·클립),
의사일정 수집, 의원 명부 동기화, 모바일 앱 QR 로그인. 자막·검색·요약·AI 는 그대로 동작합니다.

**아직 손이 필요한 것** — 다시보기(VOD)·속기록·안건 연동은 경기도의회 영상관리시스템(KMS) 규격에
맞춰져 있어, 기관 시스템에 맞는 어댑터가 필요합니다. 인증도 QR 로그인이 경기도 의정포털 앱 전용이라
기관 인증으로 바꿔야 합니다.
